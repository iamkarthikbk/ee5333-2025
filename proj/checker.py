  ...
  for net in nets:
    ...
    if len(G.nodes()):
      if not nx.is_connected(G):
        numOpens += 1
        print(f"Open net detected: {net._name}")
        ...
        # Print the first few nodes in each component to help with debugging
        for i, comp in enumerate(components):
          nodes_list = list(comp)
          print(f"  Component {i+1} sample nodes: {nodes_list[:min(3, len(nodes_list))]}")
        print("")
