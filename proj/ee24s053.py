...
# Date: 23 April 2025
#
# Declaration: The visualization function in here was written with AI assistance.
#
# Description: We first parse lef, def, guide. We extract all existing shapes
# in the def file and add it to an rtree. We sort the nets by half perimeter wirelength
# and route the smaller nets first. For routing, we first create a grid graph which honors
# obstacles in the input def file. The idea is, if two adjacent layers overlap, we interpret
# it as a connection. This is an unusual thing to do, but was a constraint in the problem statement.
# This causes shapes in adjacent layers to not be able to overlap at all, but we need to do that for
# efficiently routing nets. We create a grid graph that honors these obstacles, and uses astar to find
# the shortest valid path from the source node to the destination node. Becuase of that somewhat unusual
# constraint, we need to resort to a fallback if this routing fails (which happens very frequently). In this
# fallback approach, we continue to honor the obstacles in adjacent layers, but we relax the orientation
# constraints for routing in a certain layer. This approach can also fail, which is when we resort to a direct
# path from the source to the destination. The idea is to maximise net connections and minimize DRC violations.
# The only DRC rules being checked are minimum width and minimum spacing. Once we have a path for a net,
# we convert that path to rectangles and write it to the output def file. As for via connections, we simplyplace
# a small rectangle in the adjacent layers to indicate a connection.
...
# nets with these substrings will be skipped while routing the def file
...
# rectangle shape that is used throughout the router for obstacles and finally paths
class Rect:
...
# this is an instance in the def file
class Inst:
...
# logger that i owe my life to for helping me debug while keeping my sanity
class Logger:
...
# Function to visualize the NetworkX graph. This function was written using AI assistance.
def visualize(G, guide_shapes, pin_shapes, bbox, path=None, rectangles=None, bloated_obstacles=None, net_name=None):
    ...
    for u, v, d in G.edges(data=True):
        (via_edges if d['type'] == 'via' else wire_edges_by_layer.setdefault(u[2], [])).append((u, v))
    ...
    if rectangles:
        for layer in set(r[0] for r in rectangles):
            ...
            color, alpha = ('black', 0.8) if 'via' in layer else (layerColors[layer], 0.6)
...
# this is the output of the guide parser
class GuideRect:
...
# guide parser class assuming format doesn't change
class GuideParser:
...
# converts a given net to a rectangle
def net_to_rects(path):
    ...
    # this was the solution suggested by the prof over email. without this, open nets are reported in the checker.
    via_rectangles = []
    ...
    # since the grid graph is made up of small grid pieces we need to merge them
...
# checks if a node is blocked by an obstacle
...
# checks if an edge crosses an obstacle
                    ...
                    for line1 in layer_tracks[layer1]['lines']:
                        for line2 in layer_tracks[layer2]['lines']:
                            ...
                            if check_obstacles:
                                ...
                                x, y = node1[0], node1[1]
                                width1 = layerWidth[layer1]
                                width2 = layerWidth[layer2]
                                ...
                                for adj_layer in adjLayer[layer1]:
                                ...
                                # Check adjacent layers using layer trees directly (no bloating)
                                for adj_layer in adjLayer[layer2]:
                                    if adj_layer in layer_trees:
            ...
            for x, points in points_by_x.items():
                ...
                for i in range(len(points) - 1):
                    ...
                    if check_obstacles:
                        ...
                        # Check adjacent layers using layer trees directly (no bloating)
                        for adj_layer in adjLayer[layer]:
                            if adj_layer in layer_trees:
...
# the main course of this giant meal
def detailed_route(input_DEF, input_LEF, input_GUIDE, output_DEF, show_visualization=False):
    ...
    for net_idx, net in enumerate(net_list):
        ...
        G, _ = create_grid_graph(defr, guides_by_layer, check_obstacles=True, 
        ...
        for i in range(1, len(pin_shapes)):
            ...
            if source_node and target_node:
                ...
                try:
                    path = astar_path(G, source_node, target_node, heuristic, weight='weight')
                    path_length = astar_path_length(G, source_node, target_node, heuristic, weight='weight')
                    logger.debug(f"Found path with length {path_length}")
                    all_paths.append(path)
                    rectangles = net_to_rects(path)
                    ...
                    try:
                        ...
                        fallback_path = astar_path(fallback_G, fallback_source, fallback_target, heuristic, weight='weight')
                        path_length = astar_path_length(fallback_G, fallback_source, fallback_target, heuristic, weight='weight')
                        logger.info(f"Fallback path found with length {path_length}")
                        all_paths.append(fallback_path)
                        all_rectangles.extend(net_to_rects(fallback_path))
                        routed_pins.add((target_pin['comp_name'], target_pin['pin_name']))
                        ...
                        try:
                            ...
                            path_rects = net_to_rects(direct_path)
